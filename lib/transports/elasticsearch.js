...
class elasticsearch {
  ...
  set (data, limit, offset, callback) {
    ...
    this.version('output', err => {
      ...
      } else if (type === 'data-create') {
        this.setDataCreate(data, limit, offset, callback)
      } else if (type === 'mapping') {
...
`
  ...
  setDataCreate (data, limit, offset, callback) {
    if (data.length === 0) { return callback(null, 0) }
    ...
    data.forEach(elem => {
      const actionMeta = {create: {}}

      // use index from base otherwise fallback to elem
      actionMeta.create._index = this.base.index || elem._index

      // https://www.elastic.co/guide/en/elasticsearch/reference/master/removal-of-types.html
      if (this.ESversion < 7) {
        // use type from base otherwise fallback to elem
        actionMeta.create._type = this.base.type || elem._type
      }
      actionMeta.create._id = elem._id
          ...
          if (r.ok === true) {
            ...
            r.items.forEach(item => {
              if (item['create'].status < 400) {
                ...
                // expected if document already exists
              } else {
                console.error(item['create'])
              }
            })
          }
        }
